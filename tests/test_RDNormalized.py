...
from descriptastorus.descriptors import (
    rdDescriptors, rdNormalizedDescriptors, DescriptorGenerator, dists, hists)
...
many_smiles = "\n".join( all_smiles )
from expected_normalized_results import expected, expected_histogram

def compare_results(unit, result, idx, smiles, expected, columns, places=5):
    unit.assertEqual(result[0], expected[0])
    for i, (x,y) in enumerate(zip(result[1:], expected[1:])):
        try:
            unit.assertAlmostEqual(x,y, places, msg=f"{idx} {smiles} at %s"%columns[i+1][0])
...
class TestCase(unittest.TestCase):
    ...
    def testHaveHistogramNormalizations(self):
        missing = []
        for feature in rdDescriptors.RDKIT_PROPS[rdDescriptors.CURRENT_VERSION]:
            if feature not in hists.hists:
                missing.append(feature)
    ...
    def testHistogramNormalized(self):
        try:
            fname = tempfile.mktemp()+".smi"
            storefname = tempfile.mktemp()+".store"
            with open(fname, 'w') as f:
            ...
            opts = make_store.MakeStorageOptions( storage=storefname, smilesfile=fname,
                                                  hasHeader=False,
                                                  smilesColumn=0, nameColumn=1,
                                                  seperator=" ", descriptors="RDKit2DHistogramNormalized",
                                                  index_inchikey=True )
            make_store.make_store(opts)
            generator = DescriptorGenerator.REGISTRY["RDKit2DHistogramNormalized".lower()]
            results = []
            fails = []
            with contextlib.closing(DescriptaStore(storefname)) as store:
                for i in range(10):
                    r = store.descriptors().get(i)
                    fail = compare_results(self, r, i, all_smiles[i], expected_histogram[i], generator.GetColumns(), places=2)
                    if fail:
                        fails.append(fail)
